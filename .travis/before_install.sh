...
if [[ $TRAVIS_BRANCH == 'master' ]]; then
  ...
  echo "https://${RELEASE_GH_USERNAME}:${RELEASE_GH_TOKEN}@github.com/opencomponents/base-templates.git" > ~/.git-credentials

  npm config set //registry.npmjs.org/:_authToken=$NPM_TOKEN -q
  npm prune

  git config --global user.email "oc@opentable.com"
  git config --global user.name "OpenComponent Team"
