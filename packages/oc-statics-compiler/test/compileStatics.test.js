...
test('when oc.files.static contains a folder that doesnt exist', (done) => {
  compileStatics(withStatic(['src'], '_compile-static-package2'), (error, result) => {
...
test('when oc.files.static contain reference to a non-folder', (done) => {
  compileStatics(withStatic(['template.hbs'], '_compile-static-package3'), (error, result) => {
...
test('compile statics when oc.files.static contains valid folder and minify is false', (done) => {
  compileStatics(withStatic(['assets'], '_compile-static-package4'), (error, result) => {
    expect(error).toBeNull();
    expect(result).toBe('ok');
    nodeDir.paths(path.join(componentPath, '_compile-static-package4/assets'), (err, res) => {
      const files = res.files.sort();
      expect(
        files.map((file) => path.relative(__dirname, file).replace(/\\/g, '/'))
      ).toMatchSnapshot();
      files.forEach((file) => {
        if (!/.png$/.test(file)) {
...
test('compile statics when oc.files.static contains valid folder and minify is true', (done) => {
  const minify = true;
  compileStatics(withStatic(['assets'], '_compile-static-package5', minify), (error, result) => {
    expect(error).toBeNull();
    expect(result).toBe('ok');
    nodeDir.paths(path.join(componentPath, '_compile-static-package5/assets'), (err, res) => {
      const files = res.files.sort();
      expect(
        files.map((file) => path.relative(__dirname, file).replace(/\\/g, '/'))
      ).toMatchSnapshot();
...
test('When static files writing fails should return error', (done) => {
  ...
  const minify = true;
  compileStatics(withStatic(['assets'], '_compile-static-package6', minify), (err) => {
...
test('When static file fails to be read should return error', (done) => {
  ...
  const minify = true;
  compileStatics(withStatic(['assets'], '_compile-static-package7', minify), (err) => {
    expect(err).toMatchSnapshot();
    spy.mockRestore();
